...
class ClockConfig:     
    ...
    bg, fg = 'white', 'red'                         # face, tick colors
    hh = mh = sh = cog = 'black'  # clock hands, center
...
class AnalogDisplay(Canvas):
    ...
    def onUpdate(self, hour, mins, secs, ampm, cfg):        # on timer callback
        ...
        originX = originY = radius = cfg.size/2
        hour = hour + (mins / 60.0)
        hx, hy = self.point(hour, 12, (radius * .75), originX, originY)
        mx, my = self.point(mins, 60, (radius * .85), originX, originY)
        sx, sy = self.point(secs, 60, (radius * .90), originX, originY)
        self.hourHand = self.create_line(originX, originY, hx, hy, width=(cfg.size * .03), fill=cfg.hh)
        self.minsHand = self.create_line(originX, originY, mx, my, width=(cfg.size * .02), fill=cfg.mh)
        self.secsHand = self.create_line(originX, originY, sx, sy, width=1, fill=cfg.sh)
        cogsz = cfg.size * .01
...
class Clock(Frame):
    ...
    def makeWidgets(self, parent):
        ...
        self.dateLabel      = Label(self, bd=3, bg=self.cfg.bg, fg=self.cfg.fg)
        parent.bind('<ButtonPress-2>', self.onSwitchMode)
